...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      // Fallback if no data (e.g., direct visit)
      const timer = setTimeout(() => setLoading(false), 2000);
      return () => clearTimeout(timer);
    }
  }, []);

  const issues = analysisData?.visionIssues?.map((issue: any, index: number) => ({
    id: index,
    title: issue.title,
    severity: issue.severity.charAt(0).toUpperCase() + issue.severity.slice(1),
    color: issue.severity === 'high' ? 'text-red-500' : issue.severity === 'medium' ? 'text-orange-500' : 'text-yellow-500',
    bg: issue.severity === 'high' ? 'bg-red-500/20' : issue.severity === 'medium' ? 'bg-orange-500/20' : 'bg-yellow-500/20',
    impact: issue.description,
    ruleValid: true,
    reason: issue.suggestion,
    code: analysisData.codePatch || "/* Remediation logic loading... */",
    box: { 
      top: `${issue.boundingBox?.y || 0}%`, 
      left: `${issue.boundingBox?.x || 0}%`, 
      width: `${issue.boundingBox?.width || 10}%`, 
      height: `${issue.boundingBox?.height || 10}%` 
    }
  })) || [
    { 
      id: 0, title: "No Issues Found", severity: "Low", color: "text-emerald-500", bg: "bg-emerald-500/20",
      impact: "Run analysis to see results.", ruleValid: true,
      reason: "Please go back and enter a URL.",
      code: "/* No issues to fix */",
  ...
  const screenshotUrl = analysisData?.screenshotUrl ? `http://localhost:4000${analysisData.screenshotUrl}` : "https://images.unsplash.com/photo-1551288049-bbbda5366391?w=1200";
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-end md:items-center gap-8 border-b border-slate-800 pb-8">
          <div className="space-y-2">
            ...
            {analysisData?.url && <p className="text-cyan-400/60 font-bold text-[10px] uppercase tracking-widest">Analyzing: {analysisData.url}</p>}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <Card className="md:col-span-4 flex flex-col items-center justify-center gap-8">
            <div className="flex gap-8">
              <ScoreCircle label="UX Rating" score={analysisData?.scores?.ux || 0} color="border-cyan-400" />
              <ScoreCircle label="Accessibility" score={analysisData?.scores?.accessibility || 0} color="border-purple-500" />
            </div>
          ...
          <Card className="md:col-span-8 flex items-center gap-8 border-l-4 border-cyan-400">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 bg-slate-800 rounded text-[8px] font-black text-cyan-400 uppercase">CONTEXT: {analysisData?.projectContext || "WEB APP"}</span>
              </div>
              <p className="text-slate-400 text-[12px] leading-relaxed font-bold italic">
                {analysisData?.aiSuggestions?.[0] || "\"Ready to analyze your website. Enter a URL on the home page to begin the vision-driven UX audit.\""}
              </p>
            </div>
          </Card>
          ...
          <Card className="md:col-span-8 overflow-hidden relative min-h-[500px] border-2 border-slate-800">
            ...
            <div className="relative rounded-[2.5rem] overflow-hidden border-2 border-slate-900 bg-black group flex items-start justify-center">
              <img 
                src={screenshotUrl} 
                className={`w-full transition-all duration-1000 ${viewMode === 'heatmap' ? 'blur-md brightness-50 contrast-125' : 'opacity-80 grayscale-0'}`} 
              />
              ...
              {/* Dynamic Scan Area */}
              {issues[selectedIssue].box.width !== '0%' && (
                <motion.div 
                ...
                >
                  ...
                  <div className="absolute -top-6 left-0 bg-cyan-400 text-black px-2 py-0.5 text-[8px] font-black uppercase whitespace-nowrap">
            ...
            <Card className="h-full space-y-6">
              ...
              <div className="relative group">
                ...
                <div className="p-5 bg-black/80 rounded-2xl border border-slate-800 group-hover:border-emerald-500/30 transition-all font-mono">
                  <pre className="text-[10px] text-emerald-400 leading-relaxed overflow-x-auto whitespace-pre-wrap">
              ...
              <div className="pt-6 border-t border-slate-800 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {issues.map((issue: any, idx: number) => (
                    ...
                    >
                      <span className="text-[10px] font-black text-white uppercase truncate pr-2 text-left">{issue.title}</span>
          ...
          <Card className="md:col-span-12">
            ...
            <div className="relative aspect-[21/6] rounded-[3rem] overflow-hidden border-4 border-slate-900 group shadow-inner">
              ...
              <div className="absolute inset-0 bg-slate-900 flex items-center justify-center p-12 overflow-hidden">
                 {analysisData?.codePatch ? (
                    <div className="w-full h-full flex items-center justify-center overflow-auto" dangerouslySetInnerHTML={{ __html: analysisData.codePatch }} />
                 ) : (
              ...
              <div 
                className="absolute inset-0 bg-[#030712] flex items-center justify-center grayscale brightness-50 overflow-hidden" 
                style={{ clipPath: `inset(0 ${100 - sliderPos}% 0 0)` }}
              >
                 {analysisData?.detectedComponent ? (
                    <div className="w-full h-full flex items-center justify-center overflow-auto opacity-40" dangerouslySetInnerHTML={{ __html: analysisData.detectedComponent }} />
                 ) : (
